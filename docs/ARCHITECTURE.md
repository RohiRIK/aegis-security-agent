...
Aegis is a **thin npm package** (`aegis-security-agent`) that installs as an OpenCode plugin or a Claude Code hook and enforces a security policy on every tool call the AI agent makes. It does not run a server. It does not manage its own process. It is purely reactive — it intercepts tool calls via callbacks registered with the host (OpenCode or Claude Code) and either allows or warns them.
...
│   ├── opencode/index.d.ts      ← TypeScript types
...
The shim content (observation-only — registers no hooks, warns only):
```typescript
// Auto-generated by aegis install v0.2.0. Do not edit.
// Observation-only mode: registers no hooks, warns only.
export default async () => ({});
```

The package.json pins the exact version:
```json
{ "dependencies": { "aegis-security-agent": "0.2.0" } }
```
...
The hooks.json wires `src/hooks/pre-tool-use.ts` as a `PreToolUse` hook. Claude Code invokes this script before every tool call, passing the tool input as JSON on stdin, and expects modified (or unchanged) JSON on stdout. Always exits 0 — never blocks tool calls.
...
AegisSecurityPlugin({ directory, client })
  ...
  └── returns hook map:
       ...
       "permission.ask"               → permission handler (warning)
```
...
```
[AEGIS] Security: warned_patterns=42
```
...
Checks the command/title against `high_risk_patterns`. If matched, emits a warning to the user.
...
4. For `Bash` tool calls:
   ...
   - If install command: runs Trivy scan
5. Outputs (possibly rewritten) JSON to stdout
6. Always exits 0 — never blocks tool calls
...
Each tool call
    ...
    ├── [permission.ask]        Warn if high-risk
    └── [tool.execute.after]
...
```jsonc
{
  ...
  // Any bash command matching any of these regex patterns is warned.
  "high_risk_patterns": [
  ...
  "actions": {
    "read_file": {
      ...
      // deny_patterns: glob patterns. Any filePath matching any pattern is WARNED.
    },
    "edit_file": {
      "default": "ask",
  ...
  // Legacy — informational only, not enforced at runtime.
  "routing": {
