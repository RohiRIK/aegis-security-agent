...
> Version 0.1.9 · Last updated 2026-05-05
...
```json
["dist", "bin", "scanners-manifest.json", "aegis-policy.json", "docs/agents/aegis.md", ".claudeignore"]
```

Source files (`src/`), tests, `.github/`, and `docs/` (except `agents/`) are **not published**.
...
```
~/.aegis/bin/
├── trivy/         ← trivy binary lives here
└── trufflehog/    ← trufflehog binary lives here
```
...
| `.opencode/agents/aegis.md` | Skip if exists | `@aegis` agent persona definition |
