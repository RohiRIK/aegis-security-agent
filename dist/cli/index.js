...
async function patchOpencodeJson(targetDir) {
  ...
  if (plugins.includes("aegis-security")) {
    log("skipped", configPath + " (plugin already registered)");
    return;
  }
  config["plugin"] = [...plugins, "aegis-security"];
...
export { AegisSecurityPlugin as default } from "aegis-security";
`, OPENCODE_PKG_CONTENT;
var init_install = __esm(() => {
  init_base();
  init_ui();
  AEGIS_DIR = resolve(import.meta.dir, "../..");
  OPENCODE_PKG_CONTENT = JSON.stringify({ dependencies: { "aegis-security": "latest" } }, null, 2) + `
`;
});
